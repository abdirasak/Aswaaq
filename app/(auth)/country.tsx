...
const CountrySelection = () => {
  ...
  const { setSelectedCountry } = useAuthStore();
  const [selectedCountry, setSelectedCountryState] = useState('Egypt');
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 24, paddingVertical: 40 }}>
        ...
        <View className="gap-6 mb-12">
          {COUNTRIES.map((country) => (
            <TouchableOpacity
              key={country.id}
              onPress={() => setSelectedCountryState(country.id)}
              activeOpacity={0.7}
              className={`bg-white rounded-[32px] p-6 items-center relative shadow-sm border-2 ${
                selectedCountry === country.id ? 'border-[#A3D139]' : 'border-transparent'
              }`}
            >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          className="bg-[#064229] rounded-2xl py-5 flex-row items-center justify-center shadow-lg"
        >
